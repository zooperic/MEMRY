...
*Target: when hardware arrives (~May 1)*
*Status: PARKED — display ordered (Waveshare 3.6" Spectra 6), waiting on delivery*

### What's done (pre-hardware)
- [x] Full firmware scaffold: WiFi connect, ETag fetch, deep sleep cycle
- [x] WiFi provisioning portal (captive portal hotspot, no hardcoded creds)
- [x] Battery ADC monitoring spec'd, `bmp_render.h` written (needs color swap)
- [x] Mock server written (Node.js, zero deps)

### Do on hardware arrival
- [ ] Confirm driver chip from PCB sticker → write `color_render.h` (4bpp 6-color)
- [ ] Update display init in `memry.ino` to Spectra 6 driver
- [ ] Wire + breadboard test, run Waveshare stock demo first
- [ ] Flash MEMRY firmware, confirm color image renders
- [ ] End-to-end: upload color photo → appears on display
- [ ] Measure actual current draw in each state
...
- [ ] Upload flow: drag/drop → 6-color quantise → store BMP
...
### Image pipeline (Spectra 6 color)
- [ ] Sharp.js resize to 600x400 (cover, centred)
- [ ] Quantise to 6-color palette: Black/White/Green/Blue/Red/Yellow
- [ ] Pack as 4bpp (2 pixels per byte) for device
- [ ] Store JPEG preview separately for dashboard thumbnails
